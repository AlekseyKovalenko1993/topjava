...
@NamedQueries({
       ...
       @NamedQuery(name = Meal.UPDATE,query ="UPDATE Meal m SET m.dateTime=?1, m.calories=?2, m.description=?3 WHERE m.id=?4 AND m.user.id = ?5")
})
...
public class Meal extends AbstractBaseEntity {
    ...
    @Min(value = 1)
    @Max(value = 5000)
    @Column(name = "calories",nullable = false,columnDefinition = "amount calories of meal")
